...
SurfacePtr WindowsWindowImpl::CreateRenderSurface(Graphics::GraphicsBackend backend) {
	switch (backend) {
	case Graphics::GraphicsBackend::OpenGL: {
		auto surface = std::make_shared<Graphics::Win32OpenGLSurface>(
			hwnd_, hdc_, width_, height_
		);
		...
		return nullptr;
										  ...
										  // 未来可以添加其他后端
										  // case Graphics::GraphicsBackend::Direct3D12:
										  //     return std::make_shared<Graphics::D3D12Surface>(...);

										  // case Graphics::GraphicsBackend::Vulkan:
										  //     return std::make_shared<Graphics::VulkanSurface>(...);
